 ...
 */
typedef struct ccoll_ll_node {
    struct ccoll_ll_node *next;
    struct ccoll_ll_node *previous;
    void *value;
} ccoll_ll_node_t;
...
typedef struct ccoll_linkedlist {
    size_t length;
    ccoll_ll_node_t *head;
    ccoll_ll_node_t *tail;
 ...
 */
typedef struct ccoll_ll_iter {
    ccoll_ll_node_t *next;
    ccoll_ll_node_t *previous;
    ccoll_linkedlist_t *list;
    ccoll_ll_node_t *last_returned;
 ...
 * Returns: a pointer to the initialized list.
 */
ccoll_linkedlist_t* ccoll_ll_init_with_comparator(
    int (*compare_function)(void *value1, void *value2)
);
 ...
 * by calling ccoll_ll_drop_iter for each iterator.
 */
 ...
 * 
 * Remember to call ccoll_ll_drop_iter on the iterator to free the memory used by
 ...
 * a call to ccoll_ll_iter_next, and FALSE if not.
 */
char ccoll_ll_iter_has_next(ccoll_ll_iter_t *iter);

/*
 * Returns TRUE if the given iterator has a node available for retrieving with
 * a call to ccoll_ll_iter_previous, and FALSE if not.
 */
